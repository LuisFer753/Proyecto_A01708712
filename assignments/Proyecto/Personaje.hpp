...
class Personaje{
    ...
    public:
        ...
        int porcentajeSalud(); //este método debe devolver un int entre 0 y 100. Se calcula usando los valores de vida (que son los puntos máximos de vida que puede tener) y la salud (puntos de vida actual que tiene)
        void imprimeBarra(); //imprima su barra de vida. Su barra de vida, en realidad son 20 caracteres que indican el % de vida que tiene el personaje. Por ejemplo puedes poner un % para indicar vida y un = o un . o cualquier otro caracter que tu decidas el cual indicará lo perdido. Así un 80% de vida pudiera mostrarse como: %%%%%%%%%%%%%%%%====
        void recibeAtaque(int); //Este método recibe una cantidad de puntos que debe descontar a la salud actual de la unidad. Nunca debe quedar en negativo, si al descontar queda negativo haz que quede en 0 los puntos de salud.
        void atacar(Personaje &objetivo); //este método calcula los puntos con que dañarás al objetivo de la siguiente manera: Si el objetivo tiene un nivel más alto que esta unidad, sólo se podrá atacar con un número aleatorio entre 1 y la mitad de los puntos de ataque. Si el objetivo es de nivel más bajo o igual, se ataca con un número aleatorio entre la mitad de los puntos de ataque y el total de los puntos de ataque. Este método debe también llamar al método recibeAtaque del objetivo para que se le descuente los puntos de salud al objetivo
        void imprimir(); //despliega en pantalla características del personaje y su barra de vida
};
